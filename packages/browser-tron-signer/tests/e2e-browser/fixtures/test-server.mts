/**
 * Test server utilities for TRON e2e browser tests. Starts the HTTP server in-process on a
 * random port and exposes thin helpers around /api/test/create-request + /api/test/result/:id.
 */
...
/** Get the result of a completed request, or `{pending: true}` if still in flight. */
